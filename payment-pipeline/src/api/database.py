    ...
    with get_db() as conn:
        conn.execute("""
            CREATE TABLE IF NOT EXISTS transactions (
                ...
                oldbalance_org REAL,
                newbalance_orig REAL,
                oldbalance_dest REAL,
                newbalance_dest REAL,
                step INTEGER,
                is_anomaly INTEGER,
    ...
    with get_db() as conn:
        conn.execute("""
            ...
            (transaction_id, account_id, timestamp, transaction_type,
             ...
             oldbalance_org, newbalance_orig, oldbalance_dest, newbalance_dest, step,
             is_anomaly, anomaly_probability, processed_at)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        """, (
            tx.TransactionID,
            tx.AccountID,
